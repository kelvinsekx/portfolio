...
To get things happen quickly, let's build a simple HelloWorld with everything BASIC (no installation, downloads and server). All that will be needed is a modern browser and weak internet for now.
...
Next, within the body, let's create a div that will hold the react component. It is advisable for now that the div be empty and pass an id to make the element unique to react.
...
props is a javascript object that can be an html attribute or customized variable passed to a component. The last argument, children, is another element creatable with createElement. 
...
To compile things, we need a compiler. In our case, Babel is cool to compile JSX and let's do the compilation with Babel then. However, since  we are doing this serverlessly, let's import another link into the head.
